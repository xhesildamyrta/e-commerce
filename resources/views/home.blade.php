...
@php
$images = [
    0=>'sofaPromotional.png',
    1=>'sofaPromotional.png'
    ];
@endphp
    <div class="swiper mySwiper">
        <div class="swiper-wrapper">
            @foreach ($images as $key=>$image)
                <div class="swiper-slide hidden md:flex ">
                    <div class="w-full" style="background-color: #F2F0FF;">
                        <div class="flex  pr-44">
                            ...
                            <div class="space-y-3 py-14 xl:py-48">
                                ...
                                <h1 class="slider-heading text-black text-xl lg:text-5xl tracking-wide">New Furniture
                                    Collection Trends in 2020</h3>
                                    <p class="font-bold text-gray-400">Lorem ipsum dolor sit amet, consectetur adipiscing
                                        elit. Magna in est adipiscing in phasellus non in justo.</p>
                                    <div class="">
                                        <a href="{{ route('all-products') }}">
                                            <button
                                                class="addCart py-3 px-10 text-center text-white mt-7 tracking-wider  rounded">Shop
                                                Now </button>
                                        </a>
                                    </div>
                            </div>
    ...
    <div class="container mx-auto px-4 xl:px-32">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-y-3 gap-x-7 mt-12">
            @foreach (App\Models\Product::all()->take(4) as $product)
                <form action="{{ route('shopping-cart-store') }}" method="POST">
                    {{ csrf_field() }}
                    <x-feature_product imgSrc="{{ asset('img/' . $product->photoURL) }}" item="{{ $product->title }}"
                        code="444" price="${{ $product->price }}" />
        ...
        <div class="">
            ...
            <div class="flex text-sm  md:text-lg place-content-center space-x-1.5 sm:space-x-14 pt-5"
                style="color:#151875;">
                <!-- <div>New Arrival</div>
                <div>Best Seller</div>
                <div>Featured</div>
                ...
                <select id="catID">
                    ...
                    @foreach (App\Models\Category::all() as $cList)
                        <option class="option" id="cat{{ $cList->id }}" value="{{ $cList->id }}">{{ $cList->name }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="grid grid-cols-3 gap-x-8 gap-y-5 lg:gap-y-20 mt-4 lg:mt-16">
            @foreach (App\Models\Product::all()->where('yearOfRelease', '>=', 2021)->take(6) as $pro)
                <x-latest_product src="{{ asset('img/' . $pro->photoURL) }}" item="{{ $pro->title }}"
                    price="$ {{ $pro->price }}" delPrice="$ {{ $pro->reducedPrice }}" />
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 md:space-x-7">
            <x-shopex_offer imgSrc="img/free-delivery.png" text="24/7 Support"
                content="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Massa purus gravida." />
            <x-shopex_offer imgSrc="img/cashback 1.png" text="24/7 Support"
                content="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Massa purus gravida." />
            <x-shopex_offer imgSrc="img/premium-quality.png" text="24/7 Support"
                content="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Massa purus gravida." />
            <x-shopex_offer imgSrc="img/24-hours-support.png" text="24/7 Support"
                content="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Massa purus gravida." />
        </div>
    </div>
        ...
        <div class="w-full mt-6 lg:mt-32 ">
            <div class="lg:flex xl:px-48" style="background-color: #F1F0FF;">
                ...
                <div class="flex flex-col px-4 space-y-3.5 lg:pt-24">
                    <h1 class="section-heading text-xl mb-4 lg:text-4xl lg:mb-8">Unique Features Of leatest & Trending
                        Poducts</h1>
                    ...
                    <div class="flex  place-items-center space-x-3">
                        ...
                        <p class="text-gray-400 font-semibold">Reinforced with double wood dowels, glue, screw - nails
                            corner blocks and machine nails</p>
                    </div>
                    <div class="flex place-items-center space-x-3">
    ...
    <div class="container mx-auto px-4 xl:px-32">
        ...
        <div class="flex space-x-4">
            <div class="flex flex-col mt-12 lg:mt-28">
                ...
                <p class="mt-5 text-center text-gray-500 lg:text-left">Lorem ipsum dolor sit amet, consectetur adipiscing
                    elit.Eu eget feugiat habitasse nec, bibendum condimentum.</p>
                ...
                <div class="swiper-slide">
                    <div class="grid grid-cols-2 gap-x-3 gap-y-3 lg:grid-cols-4 mt-12">
                        ...
                        <form action="{{ route('shopping-cart-store') }}" method="POST">
                            ...
                            <input type="hidden" name="name" value="{{ $product->title }}">
                            <input type="hidden" name="price" value="{{ $product->price }}">
                            <x-top_category  imgSrc="{{ asset('img/' . $product->photoURL) }}" cat1="Mini" cat2="LCW" cat3="Chair"
                            price="{{$product->price}}" />
                        </form>
                    @endforeach
                    </div>
    ...
    <div class="h-full bg-[url('/img/nice.png')] place-content-center mt-32"
        ...
        <div class="grid place-items-center pt-11 lg:pt-44">
            <h1 class="section-heading text-center text-xl font-bold lg:text-4xl tracking-wider lg:font-medium  xl:w-1/3">
                Get Leatest Update By Subscribe Our Newslater</h1>
    ...
    <div class="container mx-auto px-4 xl:px-32">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 mt-4 lg:mt-20 gap-x-3 xl:gap-x-14 mb-28">
            @foreach (App\Models\Blog::orderBy('creation_date', 'DESC')->get()->take(3) as $blog)
                <x-latest_blog imgSrc="{{ asset('img/' . $blog->photo) }}" author="{{ $blog->user->name }}"
                    date="{{ $blog->creation_date }}" title="{{ $blog->title }}" body="{{ $blog->content }}" />
            @endforeach
        </div>
    </div>
@endsection
